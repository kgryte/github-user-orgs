...
tape( 'function returns an error to a provided callback if an error is encountered when fetching user organization memberships', function test( t ) {
	var opts;
	var get;

	get = proxyquire( './../lib/orgs.js', {
		'./factory.js': factory
	});
...
tape( 'function supports providing only a `token` option (get the authenticated user\'s organization memberships)', function test( t ) {
	var expected;
	var opts;
	var get;
...
tape( 'function supports providing both `token` and `username` options to get another user\'s organization memberships (increased rate limits)', function test( t ) {
	var expected;
	var opts;
	var get;
