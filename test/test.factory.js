...
tape( 'function returns a function which returns an error to a provided callback if an error is encountered when fetching user organization memberships', function test( t ) {
	var factory;
	var opts;
	var fcn;
...
tape( 'function returns a function which supports providing only a `token` option (get the authenticated user\'s organization memberships)', function test( t ) {
	var expected;
	var factory;
	var opts;
	var fcn;
...
tape( 'function returns a function which supports providing both `token` and `username` options to get another user\'s organization memberships (increased rate limits)', function test( t ) {
	var expected;
	var factory;
	var opts;
	var fcn;
...
tape( 'function returns a function which supports specifying the state of memberships to return', function test( t ) {
	var expected;
	var factory;
	var opts;
	var fcn;
	...
	function get( opts, clbk ) {
		t.equal( opts.query, 'state=pending', 'equal state' );
		return function get() {
			setTimeout( onTimeout, 0 );
			function onTimeout() {
				clbk( null, data, info );
